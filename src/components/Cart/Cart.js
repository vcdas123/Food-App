...
const Cart = props => {
  ...
  const confirmOrder = async userData => {
    ...
    try {
      const res = await fetch(
        "https://foodify-55a28-default-rtdb.firebaseio.com/orders.json",
        {
          method: "POST",
          headers: {
  ...
  const defaultContent = (
    <Fragment>
      ...
      {isCheckout && (
        <Checkout onCancel={props.onHideCart} confirmOrder={confirmOrder} />
      )}
      {!isCheckout && cartButtons}
    </Fragment>
  );
  const errorContent = (
    <p className={classes.orderText}>Order not placed...Try Again!</p>
  );
  ...
  const orderPlacedContent = (
    ...
    >{`Order placed successfully! (OrderID: ${orderId})`}</p>
  );
